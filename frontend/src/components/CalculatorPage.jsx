      ...
      {!showResults ? (
        <div className="w-full max-w-4xl">
          ...
          <div className="text-center mb-8">
            <h1 className="text-3xl md:text-4xl font-bold mb-3 bg-gradient-to-r from-orange-400 to-amber-400 bg-clip-text text-transparent">
              AWS Savings Calculator
            </h1>
            <p className="text-gray-400 text-sm">
              Upload your AWS bill and discover potential savings with Reserved Instances and Savings Plans
            </p>
          </div>
          
          {/* Tabs */}
          <div className="flex gap-0 mb-8 bg-zinc-900/50 rounded-xl p-1 border border-zinc-800">
            <button
              onClick={() => setActiveTab('upload')}
              className={`flex-1 py-3 px-5 text-center transition-all rounded-lg font-semibold text-sm ${
                activeTab === 'upload'
          ...
          {/* Upload Tab */}
          {activeTab === 'upload' && (
            <div className="bg-gradient-to-br from-zinc-900 to-zinc-950 rounded-xl border border-zinc-700 p-8 shadow-2xl">
              <div
              ...
              >
                ...
                <label htmlFor="file-upload" className="cursor-pointer">
                  <div className="flex flex-col items-center gap-4">
                    <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-zinc-800 to-zinc-900 flex items-center justify-center shadow-xl border border-zinc-700">
                      {isProcessing ? (
                    ...
                    <div>
                      <p className="text-base text-gray-200 mb-2 font-medium">
                        {isProcessing ? (
                          <span className="text-orange-400 font-semibold">Processing your bill...</span>
                        ) : file ? (
                      ...
                      <p className="text-sm text-gray-400">Supports CSV and PDF invoices</p>
                    </div>
                  </div>
                </label>
              </div>
              <div className="mt-6 flex items-start gap-2 text-sm text-gray-400 bg-zinc-900/50 rounded-lg p-3 border border-zinc-800">
                <FileText className="w-4 h-4 mt-0.5 flex-shrink-0 text-green-400" />
        ...
        <div className="w-full max-w-6xl py-12">
          {/* Results Header */}
          <div className="text-center mb-8">
            <div className="inline-flex items-center gap-2 bg-gradient-to-r from-green-500/20 to-emerald-500/20 border border-green-500/40 rounded-full px-5 py-2 mb-6 shadow-lg shadow-green-500/10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
              <div className="bg-gradient-to-br from-zinc-900 to-zinc-950 border border-zinc-700 rounded-xl p-5 hover:border-zinc-600 transition-all hover:shadow-xl hover:shadow-zinc-800/50">
                <h3 className="text-xs text-gray-400 mb-2 uppercase tracking-widest font-medium">Total Bill</h3>
                <div className="text-3xl font-bold text-white mb-1">
                  ${resultsData?.current_spend?.toLocaleString('en-US', { minimumFractionDigits: 0, maximumFractionDigits: 0 }) || 0}
                </div>
                <p className="text-xs text-gray-500">Current monthly spend</p>
              </div>
              
              <div className="bg-gradient-to-br from-green-950/50 to-emerald-950/50 border border-green-500/40 rounded-xl p-5 hover:border-green-500/60 transition-all hover:shadow-xl hover:shadow-green-500/20">
                <h3 className="text-xs text-green-400 mb-2 uppercase tracking-widest font-medium">Projected Cost</h3>
                <div className="text-3xl font-bold text-green-400 mb-1">
                  ${resultsData?.optimized_spend?.toLocaleString('en-US', { minimumFractionDigits: 0, maximumFractionDigits: 0 }) || 0}
                </div>
                <p className="text-xs text-green-500/80">↓ {resultsData?.savings_percentage || 0}% with RI/SP</p>
              </div>
              
              <div className="bg-gradient-to-br from-orange-950/50 to-amber-950/50 border border-orange-500/40 rounded-xl p-5 hover:border-orange-500/60 transition-all hover:shadow-xl hover:shadow-orange-500/20">
                <h3 className="text-xs text-orange-400 mb-2 uppercase tracking-widest font-medium">Monthly Savings</h3>
                <div className="text-3xl font-bold text-orange-400 mb-1">
                  ${resultsData?.monthly_savings?.toLocaleString('en-US', { minimumFractionDigits: 0, maximumFractionDigits: 0 }) || 0}
                </div>
                <p className="text-xs text-orange-500/80">Per month</p>
              </div>
              
              <div className="bg-gradient-to-br from-amber-950/50 to-yellow-950/50 border border-amber-500/40 rounded-xl p-5 hover:border-amber-500/60 transition-all hover:shadow-xl hover:shadow-amber-500/20">
                <h3 className="text-xs text-amber-400 mb-2 uppercase tracking-widest font-medium">Annual Savings</h3>
                <div className="text-3xl font-bold text-amber-400 mb-1">
                  ${resultsData?.annual_savings?.toLocaleString('en-US', { minimumFractionDigits: 0, maximumFractionDigits: 0 }) || 0}
                </div>
                <p className="text-xs text-amber-500/80">Per year</p>
              </div>
            </div>
            
            {/* Projection Details Banner */}
            <div className="relative bg-gradient-to-r from-green-500/10 via-emerald-500/10 to-orange-500/10 border border-green-500/30 rounded-xl p-6 mb-8 overflow-hidden">
            ...
            <h1 className="text-3xl md:text-4xl font-bold mb-4 bg-gradient-to-r from-orange-400 to-amber-400 bg-clip-text text-transparent">You could save</h1>
            <div className="flex items-center justify-center gap-3 text-base mb-4">
              <span className="text-orange-400 font-bold text-xl">
                {resultsData?.savings_percentage?.toFixed(1) || 0}%
              </span>
              <span className="text-gray-400 text-sm">reduction in AWS costs</span>
            </div>
            
            {resultsData?.has_reserved_instances && (
              <div className="inline-flex items-center gap-2 bg-gradient-to-r from-orange-500/20 to-amber-500/20 border border-orange-500/40 rounded-xl px-6 py-3 mt-4 shadow-lg shadow-orange-500/10">
          ...
          <div className="bg-gradient-to-br from-zinc-900 to-zinc-950 rounded-xl border border-zinc-700 overflow-hidden mb-8 shadow-2xl">
            <div className="px-6 py-4 border-b border-zinc-700 bg-zinc-900/50">
              <h2 className="text-lg font-bold text-white">Savings Breakdown</h2>
              <p className="text-xs text-gray-400 mt-1">Detailed cost analysis by service</p>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead>
                  <tr className="border-b border-zinc-700 bg-zinc-900/30">
                    <th className="px-6 py-3 text-left text-xs font-semibold text-gray-300 uppercase tracking-wide">Service</th>
                    <th className="px-6 py-3 text-right text-xs font-semibold text-gray-300 uppercase tracking-wide">Current Cost</th>
                    <th className="px-6 py-3 text-right text-xs font-semibold text-gray-300 uppercase tracking-wide">Projected Cost</th>
                    <th className="px-6 py-3 text-right text-xs font-semibold text-gray-300 uppercase tracking-wide">Savings</th>
                    <th className="px-6 py-3 text-center text-xs font-semibold text-gray-300 uppercase tracking-wide">Coverage</th>
                    <th className="px-6 py-3 text-left text-xs font-semibold text-gray-300 uppercase tracking-wide">Recommendation</th>
                  </tr>
                </thead>
                <tbody>
                      ...
                      <tr key={index} className="border-b border-zinc-800 hover:bg-gradient-to-r hover:from-zinc-800/40 hover:to-zinc-900/40 transition-all">
                        <td className="px-6 py-4">
                          <div className="flex flex-col gap-1.5">
                            <span className="text-sm text-white font-semibold">{item.service}</span>
                        ...
                        <td className="px-6 py-4 text-right">
                          <div className="flex flex-col items-end gap-1">
                            <span className="text-green-400 font-bold text-base bg-green-400/10 px-2.5 py-0.5 rounded-lg">
                        ...
                        <td className="px-6 py-4 text-center">
                          <span className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1.5 text-xs font-semibold border-2 ${
          ...
          <div className="text-center">
            <div className="flex gap-4 justify-center">
              <Button 
                ...
                className="bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700 text-white px-6 py-2.5 rounded-lg font-semibold text-sm transition-all flex items-center gap-2 shadow-lg shadow-green-600/30 hover:shadow-xl hover:shadow-green-600/40 hover:scale-105"
              >
              ...
              <Button 
                ...
                className="bg-gradient-to-r from-zinc-700 to-zinc-800 hover:from-zinc-600 hover:to-zinc-700 text-white px-6 py-2.5 rounded-lg font-semibold text-sm transition-all shadow-lg shadow-zinc-800/50 hover:shadow-xl hover:shadow-zinc-700/50 hover:scale-105"
              >
                Calculate Again
              </Button>
            </div>
